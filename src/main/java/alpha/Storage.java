...
public class Storage {
    ...
    /**
     * Loads all tasks, preserving the destination array if any record is invalid.
     *
     * @param tasks The destination array.
     * @return The number of restored tasks.
     * @throws AlphaException If the snapshot cannot be read or validated.
     */
    public int load(Task[] tasks) throws AlphaException {
        try {
            ...
            Task[] restored = new Task[tasks.length];
            int count = 0;
            for (int i = isVersioned ? firstLine + 1 : firstLine; i < lines.length; i++) {
                ...
                if (count == tasks.length) {
                    throw new IOException("More than " + tasks.length + " tasks at line " + (i + 1));
                }
                try {
                    restored[count++] = isVersioned ? parseRecord(lines[i]) : parseLegacyRecord(lines[i]);
                } catch (IllegalArgumentException exception) {
                    throw new IOException("Invalid task at line " + (i + 1) + ": " + exception.getMessage(),
                            exception);
                }
            }
            System.arraycopy(restored, 0, tasks, 0, tasks.length);
            savedBytes = bytes;
            return count;
     ...
     * @param count The number of occupied entries.
     * @throws AlphaException If the save fails or conflicts with an external edit.
     */
    public void save(Task[] tasks, int count) throws AlphaException {
        Path temporary = null;
        boolean isCommitted = false;
        try {
            if (count < 0 || count > tasks.length) {
                throw new IllegalArgumentException("Invalid task count");
            }
            ...
            for (int i = 0; i < count; i++) {
                ...
                text.append(formatRecord(tasks[i])).append('\n');
            }
